...
const AuditListPage = ({ domain, env, config, selectedIdx }) => {
  ...
  let onSelectedFilter = (filter) => {
    ...
    // Helper function to compare searchTerms arrays
    const areSearchTermsEqual = (terms1, terms2) => {
        if (terms1.length !== terms2.length) return false;
        const sorted1 = [...terms1].sort((a, b) => a.name.localeCompare(b.name));
        const sorted2 = [...terms2].sort((a, b) => a.name.localeCompare(b.name));
        return sorted1.every((term, index) =>
            term.name === sorted2[index].name &&
            term.path === sorted2[index].path &&
            JSON.stringify(term.value) === JSON.stringify(sorted2[index].value)
        );
    };

    // Check if search state has changed from last execution
    const hasSearchChanged = search.trim() !== lastExecutedSearch || !areSearchTermsEqual(searchTerms, lastExecutedSearchTerms);
        ...
        <div className="px-2 py-2 sm:px-2 lg:px-2 bg-slate-50 min-h-screen relative">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    { audits ?
                        audits.map(audit => (
                            <AuditLine audit={audit} columns={columns} key={audit.id} onClick={() => handleViewDetails(audit)} />
                        )) : null
                    }
                    </tbody>
                </table>
