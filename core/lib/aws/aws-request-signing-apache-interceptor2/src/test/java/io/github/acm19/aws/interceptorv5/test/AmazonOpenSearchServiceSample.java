...
/**
 * An AWS Request Signing Interceptor sample for arbitrary HTTP requests to an Amazon OpenSearch
 * Service domain.
 *
 * <p>In addition to the examples below that use Apache Http, the interceptor can also be used with
 * the OpenSearch REST clients for additional convenience and serialization.
 *
 * <p>Example usage with the OpenSearch low-level REST client:
 *
 * <pre>
...
public final class AmazonOpenSearchServiceSample extends Sample {
  ...
  private void indexDocumentWithCompressionEnabled() throws IOException {
    ...
    ByteArrayEntity entity =
        new ByteArrayEntity(outputStream.toByteArray(), ContentType.DEFAULT_BINARY, "gzip");
    httpPost.setEntity(entity);
    logRequest(httpPost);
  }
