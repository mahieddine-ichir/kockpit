...
import com.accor.wcp.console.services.dynaconfig.dto.PropertyChangeDto;
...
public interface PropertyChangesDtoMapper {
  ...
  default List<PropertyChangeDto> mapPropertyChanges(PropertyDocument property) {
    ...
    return property.getHistory().stream()
        .map(change -> mapPropertyChange(property.getName(), change))
        .toList();
  }
