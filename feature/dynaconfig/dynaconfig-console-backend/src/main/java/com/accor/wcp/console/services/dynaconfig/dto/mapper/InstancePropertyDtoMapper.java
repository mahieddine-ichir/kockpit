...
import static com.accor.wcp.console.services.dynaconfig.dto.Status.NOT_SYNCHRO;
import static com.accor.wcp.console.services.dynaconfig.dto.Status.SYNCHRO;
...
public interface InstancePropertyDtoMapper {

  @Mapping(target = "source", constant = "APP")
  PropertyDto mapInstanceProperty(String name, List<PropertyInstanceDto> instances);

  PropertyInstanceDto mapInstance(DynaConfigInstance propertyDocument);

  default void updatePropertiesWithInstancesInfo(
      Map<String, PropertyDto> properties, List<DynaConfigInstance> instancesProperties) {

    for (DynaConfigInstance instanceProperty : instancesProperties) {
