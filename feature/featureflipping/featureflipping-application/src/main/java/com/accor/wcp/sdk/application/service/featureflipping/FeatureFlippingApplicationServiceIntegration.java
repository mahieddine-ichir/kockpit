...
import com.accor.wcp.sdk.service.featureflipping.communication.*;
...
class FeatureFlippingApplicationServiceIntegration implements ServiceApplicationIntegration {

  private final App2WCPConsoleCommunicationService app2WCPConsoleCommunicationService;

  private final FeatureFlippingKeyUpdateHandler featureFlippingKeyUpdateHandler;

  private final FeatureFlippingKeysServiceImpl featureFlippingPropertiesService;
  ...
  void init() {
    ...
    CommandExecutor<ServiceMessageNotification> propertyUpdate = this::keyValueUpdate;
  ...
  void sendInitInstancePropertiesMessage() {
    Map<String, Set<Object>> initialPropertyValues = new HashMap<>();

    // Save current properties
    featureFlippingPropertiesService.initializeWithLocalPropertiesValues(initialPropertyValues);
  ...
  private void instanceInitPropertiesUpdateRequest(ServiceMessageNotification notification) {
        ...
        updates.stream()
            .map(
                request -> {
                  ...
                  try {
                    updateKeyWithNewValue(propertyName, newValue);
                    result = FeatureFlippingOperationResult.DONE;
                  } catch (Exception e) {
                    log.warn(
                    ...
                    result = FeatureFlippingOperationResult.ERROR;
                  }
    ...
    state =
        updateResults.stream()
            ...
            .filter(FeatureFlippingOperationResult.ERROR::equals)
            .findAny()
    ...
    if (ServiceApplicationState.INITIALIZING.equals(previousState)) {
      ...
      this.sendInitInstancePropertiesMessage();
    }
  }

  private void updateKeyWithNewValue(String propertyName, String newValue) {
    // FIXME - POC A/B Testing to review totally
    featureFlippingPropertiesService.updatePropertyWithNewValue(propertyName, newValue, new HashMap<>());
    featureFlippingKeyUpdateHandler.update(propertyName, newValue);
  }

  private void keyValueUpdate(ServiceMessageNotification notification) {
    ...
    FeatureFlippingOperationResult result;
    try {
      updateKeyWithNewValue(message.getPropertyName(), message.getNewValue());
      result = FeatureFlippingOperationResult.DONE;
    } catch (Exception e) {
      log.warn("Treat property update message in error {}. Exception: {}", message, e.getMessage(), e);
      errorMessage = e.getMessage();
      result = FeatureFlippingOperationResult.ERROR;
    }
  ...
  public void notification(ServiceMessageNotification notification) {
    log.debug("Notification for featureflipping: {}", notification);
