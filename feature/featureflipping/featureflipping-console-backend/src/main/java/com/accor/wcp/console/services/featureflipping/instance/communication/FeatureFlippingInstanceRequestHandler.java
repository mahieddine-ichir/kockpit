...
import com.accor.wcp.sdk.service.featureflipping.communication.PropertiesRefreshRequestMessageDto;
...
@Slf4j
@AllArgsConstructor
public class FeatureFlippingInstanceRequestHandler {
  private final WCPConsole2AppCommunicationService communicationService;
  private final Map<String, FeatureFlippingRequest> requests = new HashMap<>();
  ...
  public FeatureFlippingRequest getRequestById(String requestId) {
    return requests.getOrDefault(requestId, null);
  }

  public FeatureFlippingRequest linkResponseToRequest(String requestId, FeatureFlippingResponse response) {
    FeatureFlippingRequest request = this.getRequestById(requestId);
  ...
  private FeatureFlippingRequest buildDynaConfigRequest(
      String requestId,
      String domain,
      String env,
      String instanceId,
    ...
    return FeatureFlippingRequest.builder()
        .requestId(requestId)
        .domain(domain)
        .env(env)
        .appId(appId)
