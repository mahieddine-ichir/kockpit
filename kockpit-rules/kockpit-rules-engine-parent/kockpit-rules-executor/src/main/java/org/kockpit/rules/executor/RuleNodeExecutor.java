...
public class RuleNodeExecutor<T> {
  ...
  protected boolean executePredicate(
    ...
    try {
      ...
      // Warning
      log.warn("Predicate execution warning. Predicate: " + predicate, e);
      ...
      log.error("Can't execute predicate: " + predicate, t);
      StepExecution stepExecution = getStepExecution(predicate, false);
  ...
  protected void handleActions(
    ...
    if (actionExecutor == null) {
      log.warn("No action configured executor, skipping actions execution");
      return;
    }
