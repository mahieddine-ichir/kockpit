...
import static org.kockpit.rules.seemless.DynamicEndpointHelper.readDynamicEndpoint;
...
@Slf4j
class ActionSeamLessWrapper implements Action<Object> {
  private Object action;
  private Method methodSignature;
  private Action<Object> oldCompatibilityAction;
  private DocumentationDetails documentation;
  private DynamicEndpointHelper.DynamicEndpoint<org.kockpit.rules.seemless.Action> dynamicEndpoint;
  private ContextResult contextResult;
  private String propertyNameInContextToReturn;
  private boolean returnValueInContextFlag;

  ActionSeamLessWrapper(Object action, DocumentationDetails details) {
    this(action, null, details);
  }

  ActionSeamLessWrapper(Object action, Object methodSignature, DocumentationDetails details) {
    this.action = action;
  ...
  private void computeDocumentation() {
    ...
    org.kockpit.rules.seemless.Action actionAnnotation = dynamicEndpoint.getAnnotation();
  ...
  private void computeDynamicEndpointMetadata() {
    dynamicEndpoint =
        readDynamicEndpoint(action, methodSignature, org.kockpit.rules.seemless.Action.class);
  }
