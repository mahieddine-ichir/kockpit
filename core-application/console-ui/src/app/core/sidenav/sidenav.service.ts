...
export class SidenavService {
  ...
  private _env = new BehaviorSubject<any>(null);

  _env$ = this._env.asObservable();
  ...
  getEnvironmentItems(): Observable<SidenavItem[]> {
    return this.items$.pipe(map((items: SidenavItem[]) => items.filter(item => this.env && item.domain == this.env.domain && item.env == this.env.name)));
  }
  ...
  selectedSubDomain = new FormControl();

  resetSelectedSubDomain() {
    this.selectedSubDomain.reset();
    this.setSelectedSubDomain();
  }

  setSelectedSubDomain() {
    const subDomainKey = this.fetchKeyBasedOnDomainEnv() + 'subDomainSaved';
    const subDomainSaved = localStorage.getItem(subDomainKey)
    if (subDomainSaved) {
      this.selectedSubDomain.setValue(subDomainSaved);
    }
  }

  fetchKeyBasedOnDomainEnv() {
    const domainSaved = localStorage.getItem('domainSaved');
    const envSaved = localStorage.getItem('envSaved');
    return `${domainSaved}_${envSaved}_`;
  }
